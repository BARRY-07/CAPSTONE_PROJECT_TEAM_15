...
node_labels['T_LOCAL_TX_PD'] = node_labels['T_LOCAL_TX_PD'].apply(remove_sign)
...
class GCN(nn.Module):
    def __init__(self):
        ...
        self.conv2 = GCNConv(16, 1)  # Define num_classes as before
...
with torch.no_grad():
    ...
    mse = mean_squared_error(predictions[data.train_mask].cpu().numpy(), data.y[data.train_mask].cpu().numpy())
    print('Mean Squared Error (MSE) on train: {:.4f}'.format(mse))
...
with torch.no_grad():
    predictions = model(data)
    mse = mean_squared_error(predictions[data.test_mask].cpu().numpy(), data.y[data.test_mask].cpu().numpy())
    print('Mean Squared Error (MSE): {:.4f}'.format(mse))
    r2 = r2_score(predictions[data.test_mask].cpu().numpy(), data.y[data.test_mask].cpu().numpy())
    print('R2 : ', r2)